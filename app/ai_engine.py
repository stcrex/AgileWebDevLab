...
def build_ai_context(user: User) -> AIContext:
    ...
    courses = Course.query.filter_by(owner_id=user.id).order_by(Course.code.asc()).limit(20).all()
...
def build_study_reply(context: AIContext, message: str) -> str:
    ...
    The project now supports four modes:
    1. AI_PROVIDER=openai for the real OpenAI Responses API.
    2. AI_PROVIDER=ollama for a local Ollama model.
    3. AI_PROVIDER=openai_compatible for LM Studio/OpenAI-compatible local server.
    4. Built-in contextual rule engine fallback so the feature works without API keys.
    """
    provider = os.getenv("AI_PROVIDER", "local").strip().lower()
    if provider in {"openai", "responses", "openai_responses"}:
        external = try_openai_reply(context, message)
        if external:
            return external
    if provider == "ollama":
...
def try_openai_reply(context: AIContext, message: str) -> str | None:
    ...
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
            ...
            if output_text:
                return output_text
            # Fallback for older/newer shapes without output_text
            ...
            for item in outputs:
                for part in item.get("content") or []:
                    ...
                    if text_value:
                        return text_value
            return None
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, json.JSONDecodeError, OSError, IndexError, KeyError):
        return None
