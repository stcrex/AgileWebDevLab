...
def invite_group_member():
    """Invite a student into the current project group.

    For the university demo this does two useful things:
    1. If the email belongs to an existing StudySync account, that user is added
       to the current group.
    2. If the email is new, a demo student account is created with password123
       so the new member can sign in during the presentation.
    """
    membership = GroupMember.query.filter_by(user_id=current_user.id).first_or_404()
    data = request.get_json(silent=True) or request.form
    ...
    if not user:
        if not name:
            ...
            name = local_part.title() or "New Student"
        user = User(
            name=name[:80],
            uwa_id=(data.get("uwa_id") or f"INV{datetime.utcnow().strftime('%H%M%S')}")[:20],
            email=email,
            avatar_color="success",
            program="Invited StudySync Member",
            skills="Group project collaboration",
            preferred_contact="StudySync Group Chat",
        )
    ...
    invite_text = (
        f"Hi {user.name}! Join our StudySync project group '{membership.group.name}' "
        f"using group code {membership.group.code}. Demo login: {email} / password123."
    )
    action_text = "added" if added_to_group else "re-invited"
    db.session.add(
        GroupMessage(
            group_id=membership.group_id,
            sender_id=current_user.id,
            body=f"{current_user.name} {action_text} {user.name} ({email}) to the group chat as {existing_member.role}.",
        )
    )
    db.session.commit()
    ...
    return jsonify({
        ...
        "message": f"{user.name} has been {'added to' if added_to_group else 'kept in'} the group chat.",
...
def book_group_study_session():
    """Book one of the common free slots for all group members."""
    membership = GroupMember.query.filter_by(user_id=current_user.id).first_or_404()
    data = request.get_json(silent=True) or request.form
    ...
    db.session.add(
        GroupMessage(
            group_id=membership.group_id,
            sender_id=current_user.id,
            body=f"Booked {title} for {starts_at.strftime('%a %d %b, %I:%M %p')} at {location}.",
        )
    )
    db.session.commit()
    return jsonify({"ok": True, "created": created, "message": f"Booked for {created} group member(s)."})
...
def add_handbook_subject_to_courses(subject_id: int):
    subject = HandbookSubject.query.get_or_404(subject_id)
